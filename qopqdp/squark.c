...
qopqdp_squark_point(lua_State *L)
{
  ...
  GET_AS_COMPLEX(z);
  END_ARGS;
  int point[nd]; get_int_array(L, ip, nd, point);
...
void
private_squark_wall(QLA_ColorVector *s, int coords[], void *value)
{
...
qopqdp_squark_wall(lua_State *L)
{
  ...
  QLA_V_eq_zero(&data.value);
  QLA_c_eq_r_plus_ir(QLA_elem_V(data.value,color-1), z.r, z.i);
...
qopqdp_squark_s4_ferm_observables(lua_State *L)
{
  ...
  push_complex_array(L, 6, pbp_e);
  push_complex_array(L, 6, pbp_o);
